...
class Consolidation:
    ...
    def writingCsv(self, data, csv_file):
        ...
        headers = ['Fuente', 'Título', 'Contenido', 'Sección', 'Url', 'Fecha']
    ...
    def consolidate(self):
                ...
                for filename in os.listdir(folder_path):
                    if filename.endswith(".xml"):
                        ...
                        for item in items:
                            ...
                            """
                            content = item.find("description").get_text().strip()
                            title = item.find("title").get_text().strip()
                            link = item.find("link").get_text().strip()

                            date = item.find("pubDate").get_text().strip()
        ...
        if consolidated_data:
            self.writingCsv(consolidated_data, 'raw_data_corpus.csv')
